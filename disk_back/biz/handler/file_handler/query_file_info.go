...
type QueryFileInfoHandler struct {
	...
	resp     *disk_back.QueryFileInfoResponse
	dataList []*disk_back.QueryFileInfoData
	total    int64
	err      error
}
...
func NewQueryFileInfoHandler(ctx context.Context, req *disk_common.QueryFileInfoRequest) *QueryFileInfoHandler {
	return &QueryFileInfoHandler{
		ctx:  ctx,
		req:  req,
		resp: disk_back.NewQueryFileInfoResponse(),
	}
}
...
func (h *QueryFileInfoHandler) checkParam() error {
	...
	if len(param.UploadTimeInterval) > 0 {
		...
		minTime, err := localutil.ParseTime(param.UploadTimeInterval[0], string(enum.TimeLayoutCompleteMinus))
		if err != nil {
			return fmt.Errorf("上传时间设置有误,请检查开始时间, err: %v", err)
		}
		maxTime, err := localutil.ParseTime(param.UploadTimeInterval[1], string(enum.TimeLayoutCompleteMinus))
		if err != nil {
...
func (h *QueryFileInfoHandler) processBusiness() error {
	list, total, err := file_service.ProcessQueryFileInfo(h.ctx, h.req)
	if err != nil {
		return err
	}
	h.dataList = list
...
func (h *QueryFileInfoHandler) Handle() (*disk_back.QueryFileInfoResponse, error) {
	defer func() {
		h.packResp()
	}()
